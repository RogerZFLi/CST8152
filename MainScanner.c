...
rsa_int errorNumber;				/* Run-time error number = 0 by default (ANSI) */

/* External objects */
extern rsa_int line; /* Source code line numbers - defined in scanner.c */
extern Token tokenizer(rsa_nul);
 ...
 * -------------------------------------------------------------
 */
rsa_nul printScannerError(rsa_chr* fmt, ...);
rsa_nul displayScanner(Buffer* ptrBuffer);
rsa_lng getScannerFilesize(rsa_chr* fname);
