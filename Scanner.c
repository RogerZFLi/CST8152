...
rsa_int line;								/* Current line number of the source code */
extern rsa_int errorNumber;				/* Defined in platy_st.c - run-time error number */

extern rsa_int stateType[];
extern rsa_chr* keywordTable[];
extern PTR_ACCFUN finalStateTable[];
extern rsa_int transitionTable[][TABLE_COLUMNS];
...
Token tokenizer(void) {
	...
	rsa_chr c;	/* input symbol */
	rsa_int state = 0;		/* initial state of the FSM */
	rsa_int lexStart;		/* start offset of a lexeme in the input char buffer (array) */
	rsa_int lexEnd;		/* end offset of a lexeme in the input char buffer (array)*/

	rsa_int lexLength;		/* token length */
	rsa_int i;				/* counter */
	rsa_chr newc;			/* new char */
...
rsa_int nextState(rsa_int state, rsa_chr c) {
	rsa_int col;
	rsa_int next;
	col = nextClass(c);
...
Token funcID(rsa_chr lexeme[]) {
	...
	rsa_chr lastch = lexeme[length - 1];
	rsa_int isID = rsa_FALSE;
	switch (lastch) {
		case MNIDPREFIX:
...
Token funIL(rsa_chr lexeme[]) {
	...
	if (lexeme[0] != '\0' && strlen(lexeme) > NUM_LEN) {
		...
		if (tint >= 0 && tint <= SHRT_MAX) {
			...
			currentToken.attribute.intValue = (rsa_int)tint;
...
Token funcSL(rsa_chr lexeme[]) {
	Token currentToken = { 0 };
	rsa_int i = 0, len = (rsa_int)strlen(lexeme);
...
rsa_nul printToken(Token t) {
	extern rsa_chr* keywordTable[]; /* link to keyword table in */
	switch (t.code) {
	case RTE_T:
